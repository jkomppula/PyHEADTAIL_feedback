...
#
#
#         for idx, slice_set in enumerate(processor_slice_sets):
#             idx_from = idx * self._n_slices_per_bunch
#             idx_to = (idx + 1) * self._n_slices_per_bunch
#
#             np.copyto(self._input_signal_x[idx_from:idx_to],slice_set.mean_x)
#             np.copyto(self._input_signal_y[idx_from:idx_to],slice_set.mean_y)
#
#
#         bin_edges_x = np.copy(self._input_bin_edges)
#         signal_x = np.copy(self._input_signal_x)
#
#         for processor in self._processors_x:
#             bin_edges_x, signal_x = processor.process(bin_edges_x, signal_x,processor_slice_sets,
#                                                      phase_advance=self._phase_advance_x)
#
#         bin_edges_y = np.copy(self._input_bin_edges)
#         signal_y = np.copy(self._input_signal_y)
#         for processor in self._processors_y:
#             bin_edges_y, signal_y = processor.process(bin_edges_y, signal_y,processor_slice_sets,
#                                                      phase_advance=self._phase_advance_y)
#
#     def _init_variables(self,bunch):
#         if self._mpi:
