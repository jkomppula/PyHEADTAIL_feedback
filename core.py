...
utilize the basic functions and tools from the core (this file). The signal
from the interfaces is processed by passing it hrough a list of signal
processors, which represents, for example, a model for a transverse feedback
...
axamples for signals and signal processors. The core functions for processing
...
=========================
There is not only one definition for a signal. For example. a signal might be
a continuous, time varying, quantity, i.e. an analog signal. In another
context it might be a list of numbers which represent values of certain
moments of equally spaced time, i.e. a digital signal. There are also
situations, where signals are more complex.

Unfortunately, the required definition of a signal should work in all
of the mentioned cases. Because it is very unpractical if not almost
impossible to develop all signal processors to work with the all types
of signals, different definitions for a signal can be used in the framework.

The basic definition of a signal is that it consists of discrete numbers
in time domain. More specifically, a signal is is a numpy array of
...
process signals from higher classes.
    ...
    In practice this means that signals from each bunch has an equal number of
    equally spaced slices/samples.
...
information is given in paralelle to the signal to the signal processors
...
specific processors for the signal conversion with the time domain signals are
programmed.
...
data visualization (debug extension). Names of the extensions supported by the
processor are listed on the variable extensions.
...
framework.
    ...
    in processor. Because calculations of the statistical variables of the
...
"""
    ...
    the signal without modifications. A combiner uses a ragister as a signal
    ...
    ### Macro extension
    -------------------
    The debug extension adds two features to the processors. Firstly, it
    allows usage of macros to automize creation of features to processors.
    Secondly, it save input and output parameters and signals into
    variables input_parameters, input_signal, output_parameters and
    output_signal, which can be riden outside after the signal processing.

    All the items in the list _macros must be functions, which can handle the
    following input parameters (self, parameters, signal, output_parameters,
    output_signal, *args, **kwargs)

    It is recomended to include the debug extension to the all programmed
    signal processors.
"""
...
def debug_macro(obj, **kwargs):
    """
    A debug macro, which can be added to the macro list of the signal
    processor. If input parameter debug = True is given to the signal
    ...
    -------
    object
        A macro object, which is run in process(...) method
    """
